      ...
      >
        ...
        <template v-slot:[`item.status`]="{ item }">
          <v-chip small outlined color="purple">
            ...
            <v-icon size="16" v-else-if="item.status === 'pending'"
              >mdi-clock</v-icon
            >
            <v-icon size="16" v-else-if="item.status === 'reproved'"
              >mdi-close</v-icon
            >
        ...
        <template v-slot:no-data>
          <div class="px-4" v-if="!loading">
            ...
            <v-icon color="grey" @click="reloadPendingAffiliate"
              >mdi-reload</v-icon
            >
          </div>
      ...
      <v-dialog persistent v-model="dialogRequest" width="400">
        <v-card dark>
          <v-card-title class="purple--text">
            <v-icon @click="closeDialogList" color="purple">mdi-close</v-icon
          ...
          <v-card-text class="text-center">
            <v-container>
              <v-avatar size="60">
                <v-img
                  src="../../../../assets/userPhotos//user-default.png"
                  contain
                  cover
                ></v-img>
              </v-avatar>
              <v-col>
                <v-btn dark color="purple" rounded>{{
                  userAffiliateList.user
                }}</v-btn>
              ...
              <v-col>
                ...
                <p class="caption grey--text mt-5">
                  Usuário para checkout escolhido
                </p>
                <v-btn class="mt-n5" color="purple" outlined small rounded
                  >@L4rissa</v-btn
                >
              </v-col>
            </v-container>
          ...
          <v-card-actions>
            <v-btn text color="purple" @click="recusedItem(userAffiliateList.id)"
              >RECUSAR</v-btn
            >
            <v-btn text color="purple" @click="approvedItem(userAffiliateList.id)"
              >APROVAR</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      async ListPendingAffiliate() {
        try {
          const pendingRequests =
            await affiliateService.listPendingAffiliateRequests();
