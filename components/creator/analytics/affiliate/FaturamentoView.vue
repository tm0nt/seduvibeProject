<template>
  <v-card color="input_color" height="400" max-width="400" class="rounded-xl mt-n3 pb-1" flat>
    <v-toolbar flat color="transparent">
      <v-toolbar-title class="mt-n2">Você já recebeu</v-toolbar-title>
    </v-toolbar>
    <v-card color="primary" class="rounded-xl mx-2 pa-1 mt-n3" flat>
      <span class="text-caption ma-2">Sua comissão</span>
    ...
    <v-list bg-color="input_color">
      <v-list-item v-for="salesAll in salesList" :key="salesAll.id">
        <v-list-item-content>
          <v-list-item-title class="white--text">{{ salesAll.userId }}</v-list-item-title>
          <v-list-item-subtitle class="white--text">
            {{
              parseFloat(salesAll?.affiliateCreatorData?.productValue).toLocaleString("pt-BR", {
                style: "currency",
                currency: "BRL",
              })
            }}
            - ({{
              parseFloat(
                salesAll?.commission * salesAll.affiliateCreatorData?.productValue
    ...
    <v-card-actions>
      <v-btn block class="elevation-0 text-capitalize" color="input_color" @click="moreVendas"
...
export default {
  ...
  methods: {
    ...
    async listSales() {
      try {
        const response = await affiliateService.latestSales(this.$store.state.id);
