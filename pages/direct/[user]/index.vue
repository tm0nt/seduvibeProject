        ...
        <VCol cols="12" md="8" offset-md="2">
          ...
          <VCard color="background" class="elevation-8 rounded-xl mx-auto" width="400" flat>
            <VCard-text class="text-center">
              ...
              <VSpacer></VSpacer>
              <VTextField class="mt-4" variant="solo" v-model="donationValue" bg-color="input_color" label="Valor" type="number" hide-spin-buttons prepend-inner-icon="mdi-coin"></VTextField>
              <PaymentMethod />
            </VCard-text>
          </VCard>
        </VCol>
  ...
  const fetchData = async () => {
    try {
      ...
      const { data, error } = await useFetch(
        `https://api.seduvibe.com/list_creator/${user.value}`,
        {
          method: "GET",
          headers: {
